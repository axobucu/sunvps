...
// === Token SunWin (bạn thay bằng token thật) ===
...
let clients = new Set(); // lưu client kết nối tới WS riêng
    ...
    try {
      ...
      if (Array.isArray(json) && json[3]?.res?.d1) {
        ...
        if (!rikCurrentSession || res.sid > rikCurrentSession) {
          ...
          console.log(`📥 Phiên mới ${res.sid} → ${newResult.ket_qua}`);

          // broadcast cho WS client của bạn
          for (const ws of clients) {
            if (ws.readyState === WebSocket.OPEN) {
              ws.send(JSON.stringify(newResult));
            }
          }

          // reconnect lại sau mỗi phiên
          setTimeout(() => {
            rikWS?.close();
            connectRikWebSocket();
          }, 1000);
...
fastify.get("/api/taixiu/sunwin", async () => {
  const valid = rikResults.filter((r) => r.xuc_xac_1 && r.xuc_xac_2 && r.xuc_xac_3);
...
const start = async () => {
  try {
    ...
    console.log(`🚀 HTTP API chạy tại ${address}`);

    // Tạo WS server riêng cho bạn
    const wss = new WebSocket.Server({ server: fastify.server, path: "/ws" });

    wss.on("connection", (ws) => {
      console.log("⚡ Client kết nối WS riêng");
      clients.add(ws);
